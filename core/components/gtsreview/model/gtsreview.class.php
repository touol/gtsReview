...
class gtsReview
{
    ...
    {
        $this->modx =& $modx;
        $corePath = MODX_CORE_PATH . 'components/gtsreview/';
        $assetsUrl = MODX_ASSETS_URL . 'components/gtsreview/';
        ...
        $this->modx->addPackage('gtsreview', $this->config['modelPath']);
        $this->modx->lexicon->load('gtsreview:default');
        ...
        switch ($ctx) {
            ...
            default:
                if (!defined('MODX_API_MODE') || !MODX_API_MODE) {
                    ...
                    if ($css = $this->modx->getOption('gtsreview_frontend_css')) {
                        $this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
                    }

                    $config_js = preg_replace(array('/^\n/', '/\t{5}/'), '', '
							gtsReview = {};
							gtsReviewConfig = ' . $this->modx->toJSON($this->config) . ';
					');


                    $this->modx->regClientStartupScript("<script type=\"text/javascript\">\n" . $config_js . "\n</script>", true);
                    if ($js = trim($this->modx->getOption('gtsreview_frontend_js'))) {
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->config['processorsPath'])
            ...
            : MODX_CORE_PATH . 'components/gtsreview/processors/';
