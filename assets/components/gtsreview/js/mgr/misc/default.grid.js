gtsReview.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper gtsreview-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('gtsreview-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('gtsreview-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('gtsreview-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('gtsreview-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    gtsReview.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(gtsReview.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = gtsReview.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('gtsreview-grid-default', gtsReview.grid.Default);